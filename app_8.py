...
def extract_movie_id_from_filename(filename):
    ...
    if len(parts) >= 2:
        # La partie avant le premier "-" est considérée comme le movieId
        movie_id_part = parts[0]
        # Extraire le movieId de la partie avant le premier "_"
        movie_id = movie_id_part.strip()  # Supprimer les espaces éventuels autour du movieId
        return movie_id
    else:
        return None
