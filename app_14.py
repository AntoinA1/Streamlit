...
# Si SVD est sélectionné
elif rec_type == "Latent Factor Model":
    # Affichage de la barre latérale pour les options
    st.sidebar.title("Options")

    # Paramètres de l'algorithme SVD
    ...
    # Obtenir les identifiants uniques des utilisateurs
    unique_users = sorted(set([user_id for user_id, *_ in ratings_data.raw_ratings]))

    # Limiter le nombre d'utilisateurs affichés à 10 pour des raisons d'interface utilisateur
    max_users_to_display = 700

    # Sélectionner les utilisateurs à afficher
    users_to_display = unique_users[:max_users_to_display]

    # Section pour choisir l'utilisateur
