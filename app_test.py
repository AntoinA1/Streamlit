...
# Inject CSS to change background and add custom styles
st.markdown(
    """
    <style>
    .stApp {
...
def sidebar(rec_type):
    ...
    if rec_type == "User-Based":
        st.sidebar.write("Options coming soon...")
...
st.markdown('<div class="title"><h1>Recommender System, Group 6</h1></div>', unsafe_allow_html=True)
...
if st.button("Rec Sys Explanation", key="Explanation button"):
    st.markdown('<div class="section">' + explication + '</div>', unsafe_allow_html=True)

# Section pour choisir le type de système de recommandation
st.markdown('<div class="section"><h2>Choose one Recommender System</h2></div>', unsafe_allow_html=True)
...
if rec_type == "User-Based":
    ...
    st.markdown('<div class="section"><h2>New User - Create a profile</h2></div>', unsafe_allow_html=True)
    ...
    if st.session_state['new_ratings']:
        st.markdown('<div class="section"><h2>Rated films</h2></div>', unsafe_allow_html=True)
    ...
    if st.button("Get recommendations"):
        if st.session_state['new_ratings']:
            ...
            if recommendations:
                st.markdown('<div class="section"><h2>Personalized recommendations</h2></div>', unsafe_allow_html=True)
                for movie_id, rating in recommendations:
                    movie_title = get_movie_title_by_id(movie_id, movie_id_to_title)
                    st.markdown(f'<div class="recommendation">Recommended film : {movie_title}, Estimation : {rating:.2f}</div>', unsafe_allow_html=True)
            else:
...
# Si Content-Based est sélectionné
    ...
    st.markdown('<div class="section"><h2>Which film did you like?</h2></div>', unsafe_allow_html=True)
